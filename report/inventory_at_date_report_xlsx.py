...
class InventoryAtDateReportXlsx(ReportXlsx):
    ...
    def generate_xlsx_report(self, workbook, data, stock_history):        
        ...
        config_parameter_attrs = self.env.ref(
            'stock_inventory_at_date_with_variants_report_xlsx.config_inventory_at_date_report_xlsx_variants_attrs')

        if config_parameter_attrs:
            attrs = [
                attr.strip(' ')
                for attr in config_parameter_attrs.value.split(',')
                ]
